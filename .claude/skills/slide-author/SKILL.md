...
### 1. 설계안 읽기
`docs/course-plan.md`에서 해당 장표(NN) 항목을 찾아 다음을 추출한다:
제목 / 핵심 내용 / **스토리 장면** / 전달 메시지·대사 / 화면 구성 / 연관 장표 / 중요도 / 배정 시간 / 추가 이야기.

- 설계안의 "화면 구성"은 **의도**이지 마크업 지시가 아니다. DESIGN.md 컴포넌트로 번역한다.
- 설계안 내용이 `docs/verification-notes.md`와 충돌하면 **verification-notes가 이긴다** (예: 설계안 39의 "Receive Task" → Camunda 7은 Intermediate Catch Event만).
...
### 7. 리뷰 → 완료
1. `slide-reviewer` 서브에이전트를 실행한다. 프롬프트에 **장표 번호(설계안 NN)와 section 위치(#/h)** 를 명시.
2. FAIL 항목을 고치고 재검증. 모두 PASS + 브라우저 검증 통과 후에만 완료 선언.
3. 커밋 메시지에 설계안 번호를 남긴다 (예: `설계안 21 Start Event 장표 추가`).
